...
### G. Цикл, untrusted і трейс — `npm run try-loop` (43 перевірки)
...
### J. Планувальник — `npm run try-watch` (56 перевірок)
...
- [ ] прийнята картка (`Done` на дошці) не опитується
- [ ] картка в `In review` опитується: доопрацювання коментарем працює
- [ ] обірваний прогін опитується попри колонку, але не довше вікна
- [ ] типово правка йде без питання: ворота вимкнені
- [ ] з `confirmWrites` `write_file` зупиняється на воротах
- [ ] `devflow reply` з термінала пише в робочу копію, не в теку користувача
...
- [ ] вичерпаний ліміт кроків продовжується коментарем, історія збережена
...
### M. Робоча копія — `npm run try-workspace` (15 перевірок)

- [ ] `git worktree` дає окрему копію зі спільною історією
- [ ] `node_modules` видно з копії, і він не робить її «брудною»
